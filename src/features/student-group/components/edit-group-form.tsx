...
interface Props {
    ...
    initialValues: {id: number, name: string, section: string, year: string, specialtyId: number};
    onCancel?: () => void;
}

export function GroupForm({initialValues, specialtyOptions, onCancel }: Props) {

    const form = useForm<z.infer<typeof createGroupSchima>>({
        defaultValues: {
            name: initialValues.name,
            section: initialValues.section as SECTION,
            year: initialValues.year as YEAR,
            specialtyId: initialValues.specialtyId.toString(),
